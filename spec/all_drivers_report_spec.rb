...
RSpec.describe AllDriversReport do
  describe "#generate" do
    it "can handle multiple registrations" do
      report = AllDriversReport.new(filepath: "./example_inputs/multiple_registrations.txt").generate
    ...
    it "can handle trips without registered drivers" do
      report = AllDriversReport.new(filepath: "./example_inputs/not_registered.txt").generate

      expect(report).to eq("")
    end

    it "generates a typical report" do
      report = AllDriversReport.new(filepath: "./example_inputs/problem_statement.txt").generate
    ...
    it "returns the expected number of drivers for large datasets" do
      report = AllDriversReport.new(filepath: "./example_inputs/large_dataset_50.txt").generate

      expect(report.split("\n").count).to eq(50)
    end

    it "filters out trips that are too fast or slow" do
      report = AllDriversReport.new(filepath: "./example_inputs/too_fast_or_slow.txt").generate
